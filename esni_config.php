                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editModal"
                                    ...
                                    data-titulo="<?= clean($s['titulo']) ?>" data-descripcion="<?= clean($s['descripcion']) ?>"
...
elseif ($tab === 'lineas'):
    ...
    $lineas = $pdo->query("SELECT l.*, v.codigo AS vacuna, v.color AS vacuna_color, d.codigo AS dosis, g.codigo AS grupo_edad, s.codigo AS seccion_codigo, s.titulo AS seccion_titulo
                    ...
                    <div class="row">
                        <div class="col-6 mb-2"><label class="form-label small fw-semibold">Sexo</label>
                            <select name="sexo" class="form-select form-select-sm">
                                <option value="A">Ambos</option><option value="M">Mujer</option><option value="F">Varon</option>
                            </select>
                        </div>
                ...
                <table class="table table-sm mb-0" style="font-size:.78rem">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><small class="mono-pill"><?= clean($l['dosis'] ?? '-') ?></small></td>
                            <td><small class="mono-pill"><?= clean($l['grupo_edad'] ?? '-') ?></small></td>
                            <td class="text-center"><?= $l['sexo'] === 'M' ? '<span class="badge bg-info">M</span>' : ($l['sexo'] === 'F' ? '<span class="badge bg-warning text-dark">V</span>' : 'A') ?></td>
                            <td class="text-end">
        ...
        <div class="config-card">
            ...
            <div class="card-body">
                <form method="POST">
                    ...
                    <div class="mb-2"><label class="form-label small fw-semibold">Clave</label><input type="text" name="clave" class="form-control form-control-sm" required placeholder="ej: tabla_origen, columna_cod_item"></div>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editModal"
                                    ...
                                    data-descripcion="<?= clean($p['descripcion']) ?>"><i class="fas fa-edit"></i></button>
...
document.getElementById('editModal').addEventListener('show.bs.modal', function (event) {
    ...
    if (tipo === 'vacuna') {
        ...
        title.textContent = 'Editar Dosis';
        html = `<form method="POST">
        ...
        title.textContent = 'Editar Grupo de Edad';
        html = `<form method="POST">
        ...
        html = `<form method="POST">
            ...
            <div class="mb-2"><label class="form-label small fw-semibold">Descripcion</label><textarea name="descripcion" class="form-control form-control-sm" rows="2">${v('descripcion')}</textarea></div>
            <div class="mb-2"><label class="form-label small fw-semibold">Layout</label>
                <select name="layout" class="form-select form-select-sm">
        ...
        html = `<form method="POST">
            ...
            <div class="row"><div class="col-6 mb-2"><label class="form-label small fw-semibold">Sexo</label>
                <select name="sexo" class="form-select form-select-sm">
                    ...
                    <option value="M" ${v('sexo')==='M'?'selected':''}>Mujer</option>
                    <option value="F" ${v('sexo')==='F'?'selected':''}>Varon</option>
                </select></div>
